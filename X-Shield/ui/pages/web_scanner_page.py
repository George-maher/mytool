...
class WebScannerPage(QWidget):
    """Web Scanner page with threading and terminal output"""
    ...
    def setup_terminal_output(self, parent_layout):
        ...
        # Add initial message
        self.add_terminal_message("INFO", "Ready to scan web applications")
    ...
    def start_scan(self):
        """Start web scan via ModuleManager"""
        ...
        if not success:
            self.add_terminal_message("ERROR", "Failed to start web scan")
            self.on_finished()

    def stop_scan(self):
        """Stop web scan"""
        if self.module_manager:
            self.module_manager.stop_module("web_scanner")
    ...
    def on_progress(self, module_name, current, total):
        ...
        if module_name == "web_scanner":
            ...
            self.status_label.setText(f"Status: Auditing... {percentage}%")
